...
class App:
    def __init__(self, root):
        ...
        tk.Radiobutton(
            options_frame, text="TeX", variable=self.format_var, value="tex"
        ).pack(side=tk.LEFT)
        tk.Radiobutton(
            options_frame, text="Plain", variable=self.format_var, value="plain"
        ).pack(side=tk.LEFT)

        input_frame = tk.LabelFrame(
            main_frame, text="元の文献情報を入力", padx=5, pady=5
        )
        ...
        self.convert_button = tk.Button(
            main_frame, text="変換実行", command=self.process_text
        )
        self.convert_button.pack(fill=tk.X, pady=5)

        output_frame = tk.LabelFrame(
            main_frame,
            text="変換後の文献情報 (クリップボードにコピーされます)",
            padx=5,
            pady=5,
        )
...
if __name__ == "__main__":
    ...
    if load_rules():
        ...
        messagebox.showerror(
            "起動エラー",
            f"ルールファイル ({DEL_FILENAME}, {REP_FILENAME}) の読み込みに失敗しました。\nファイルがアプリケーションと同じディレクトリに存在するか確認してください。",
        )
        root.destroy()
